...
export default function BlogComments({ comments = [] }: CommentProps) {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-6">
            <h2 className="text_gradient w-fit bg-clip-text text-transparent text-2xl font-bold mb-6 text_gradient">Comments</h2>
                        ...
                        {comment.replies && comment.replies.length > 0 && (
                            <div className="ml-8 mt-4 space-y-4 border-l-2 border-graphite-gray pl-4">
                                {comment.replies.map((reply, replyIndex) => (
                                    <div key={replyIndex} className="border-2 border-graphite-gray rounded-xl p-3 hover:bg-graphite-gray/10 transition-colors duration-200">
