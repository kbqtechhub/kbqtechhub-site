...
export const getAllBlogs = cache(async (options?: {
    revalidate?: number
}) => {
    try {
        // Implement dynamic revalidation
        const revalidateDuration = options?.revalidate ?? 3600 // 1 hour default
...
// Pagination support
export const getBlogsPaginated = cache(async (options: {
    page?: number
    limit?: number
    category?: string
} = {}) => {
